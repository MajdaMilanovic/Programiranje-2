...
public:
	...
	bool operator==(const ZadovoljstvoKupca& k)
	{
		if (_ocjena != k._ocjena)
			return false;
		for (size_t i = 0; i < _komentariKarakteristika.getTrenutno(); i++)
		{
			if (!(_komentariKarakteristika.getElement1(i) == k._komentariKarakteristika.getElement1(i) &&
...
public:
	...
	Kupac(const Kupac& obj) : Osoba(obj._imePrezime, obj._datumRodjenja)
	{
	...
	void DodajKupovinu(float iznos, ZadovoljstvoKupca& z)
	{
		...
		if (_bodovi.size() != 0)
		{
			...
			if (zadnjiBodovi > 5)
			{
				...
				thread t(&Kupac::posaljiMail, this, zadnjiBodovi);
				t.join();
			}
		}
